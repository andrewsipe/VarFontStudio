...
def _write_name_records(font: TTFont, axis_defs: List[AxisDef], plan: NameIDPlan) -> None:
    ...
    for patch in plan.windows_name_patches or []:
        try:
            ...
            continue
        if name_id < 0 or name_id > 25 or name_id == 15:
            continue
        ...
        value = str(text)
        if name_id == 25:
            # Empty ID 25 patch means omit from export; family_ps_prefix stays for naming.
            if value == "":
                omit_name_id_25 = True
                name_table.removeNames(
                    nameID=25, platformID=3, platEncID=1, langID=0x0409
                )
            continue
        if value == "":
    ...
    if prefix and not omit_name_id_25:
